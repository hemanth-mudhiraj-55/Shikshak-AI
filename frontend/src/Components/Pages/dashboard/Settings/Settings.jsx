...
import { SERVER_BASE_URL } from '../../../../services/apiConstants.js';
...
const Settings = ({ collapsed }) => {
  ...
  // User data (local settings that don't come from backend)
  ...
  // Sync profileData into userData and formData when profileData loads
  useEffect(() => {
    if (profileData.username || profileData.email) {
      const stored = authService.getStoredUser();
      const prefs = stored?.preferences || {};
      const nextTheme = prefs.theme || theme || 'system';
      ...
      setUserData(prev => ({
        ...
        notifications: {
          ...prev.notifications,
          email: prefs.notifications?.email ?? prev.notifications.email,
          push: prefs.notifications?.push ?? prev.notifications.push,
        },
        privacy: {
          ...prev.privacy,
          readReceipts: prefs.privacy?.readReceipts ?? prev.privacy.readReceipts,
          onlineStatus: prefs.privacy?.onlineStatus ?? prev.privacy.onlineStatus,
        },
        accessibility: {
          ...(prev.accessibility || {}),
          reduceMotion: prefs.accessibility?.reduceMotion ?? prev.accessibility?.reduceMotion ?? false,
          highContrast: prefs.accessibility?.highContrast ?? prev.accessibility?.highContrast ?? false,
        }
      }));
      setFormData(prev => ({
        ...prev,
    ...
    try {
      ...
      if (response.success) {
        ...
        setProfileData(prev => ({
          ...
          phone: formData.phone,
          bio: formData.bio,
          profilePicture: response.data?.data?.user?.profilePicture ?? prev.profilePicture
        }));
        setIsEditing(false);
      ...
      setInlineError(error.message || 'Failed to save profile');
      setSaveStatus('error');
      setTimeout(() => setSaveStatus(null), 3000);
    }
  };
  ...
  const handleSavePreferences = async () => {
    try {
      ...
      const response = await authService.updatePreferences({
        ...
        privacy: {
          readReceipts: !!formData.privacy?.readReceipts,
          onlineStatus: !!formData.privacy?.onlineStatus,
        },
        accessibility: {
          reduceMotion: !!formData.accessibility?.reduceMotion,
          highContrast: !!formData.accessibility?.highContrast,
        }
      });
      if (response.success) {
        setUserData(formData);
      ...
      setInlineError(error.message || 'Failed to save preferences');
      setSaveStatus('error');
      setTimeout(() => setSaveStatus(null), 3000);
    }
  };
  ...
  const handleSave = async () => {
    ...
    if (activeTab === 'notifications' || activeTab === 'privacy' || activeTab === 'appearance' || activeTab === 'preferences') {
  ...
  const handlePasswordSubmit = async (e) => {
    ...
    try {
      ...
      setSaveStatus('error');
      setInlineError(error.message || 'Failed to change password');
      setTimeout(() => setSaveStatus(null), 3000);
    }
  };
  ...
  const handleProfilePictureChange = async (e) => {
    ...
    if (file) {
      try {
        ...
        const newPic = response.data?.data?.user?.profilePicture;
        if (newPic) {
          setFormData(prev => ({ ...prev, profilePicture: newPic }));
          setUserData(prev => ({ ...prev, profilePicture: newPic }));
          setProfileData(prev => ({ ...prev, profilePicture: newPic }));
        }
        setSaveStatus('success');
        setTimeout(() => setSaveStatus(null), 2500);
      } catch (error) {
        setInlineError(error.message || 'Failed to upload profile picture');
        setSaveStatus('error');
        setTimeout(() => setSaveStatus(null), 3000);
      }
    }
  ...
  const toProfileSrc = (value) => {
    if (!value) return userData.profilePicture;
              ...
              <div className="settings-section">
                ...
                <div className="profile-picture-section">
                  <div className="profile-picture-wrapper">
	                    <img 
	                      src={toProfileSrc(formData.profilePicture)} 
	                      alt="Profile" 
                    ...
                    <label className={`theme-option ${formData.theme === 'light' ? 'theme-option-active' : ''}`}>
                      <input
                        ...
                        name="theme"
	                        ...
	                        onChange={() => { setFormData({ ...formData, theme: 'light' }); setTheme('light'); }}
	                      />
                      <Sun size={24} />
                      <span>Light</span>
                    </label>
                    ...
                    <label className={`theme-option ${formData.theme === 'dark' ? 'theme-option-active' : ''}`}>
                      <input
                        ...
                        name="theme"
	                        ...
	                        onChange={() => { setFormData({ ...formData, theme: 'dark' }); setTheme('dark'); }}
	                      />
                      <Moon size={24} />
                      <span>Dark</span>
                    </label>
                    ...
                    <label className={`theme-option ${formData.theme === 'system' ? 'theme-option-active' : ''}`}>
                      <input
                        ...
                        name="theme"
	                        ...
	                        onChange={() => { setFormData({ ...formData, theme: 'system' }); setTheme('system'); }}
	                      />
                      <Globe size={24} />
                      <span>System</span>
                    </label>
