...
class EmailService {
  constructor() {
    ...
    this.fromAddress = process.env.EMAIL_FROM || process.env.EMAIL_USER || 'noreply@localhost';
  ...
  async sendOTP(email, otp) {
    const mailOptions = {
      from: `"ShikshakAI" <${this.fromAddress}>`,
      to: email,
    ...
    try {
      if (!this.transporter) {
        console.log(`[Local email preview] OTP for ${email}: ${otp}`);
        return true;
      }
  ...
  async sendWelcomeEmail(email, username) {
    ...
    try {
      if (!this.transporter) {
        console.log(`[Local email preview] Welcome email queued for ${email}`);
        return true;
      }
      await this.transporter.sendMail(mailOptions);
      return true;
